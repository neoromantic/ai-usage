...
- **The device cap can be exceeded briefly when new devices write at once.** Counting and then writing is not atomic on the REST store. A new device counts the team again after its write, and one that is not among the devices stored first deletes its record and gets 403, so the team read and the writes agree. If that delete fails, the record stays unlisted until it expires. A Lua script or `SET NX` per slot would make the cap exact.
