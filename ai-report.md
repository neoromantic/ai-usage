...
- **Probes** only ask the harnesses, from the user's home directory:
  - Claude: `claude auth status --json`, and Claude Code's own config and cached usage. Never the keychain or `.credentials.json`. Claude Code writes that cache only when it reads the usage, as its /usage dialog does, so a person who never opens /usage had no reading at all. So when a claude.ai subscription's cache is missing, another account's, or at least 10 minutes old, the probe first runs `claude -p /usage --no-session-persistence --model ai-usage-no-model --settings '{"disableAllHooks":true}'`, once per home and run. It gets the home, `PATH`, and working directory `auth status` gets, no input, and `DISABLE_AUTOUPDATER=1`, and not `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`, since with it Claude Code does not read the usage. In print mode /usage is a local command: it reads the usage, caches it, and prints it, and it calls no model, so it spends no token. The model named does not exist. /usage ignores it, and a Claude Code too old to run /usage this way, which would send it to a model as a prompt, fails at once with nothing spent. It leaves no session, runs no hooks, and takes a few seconds; after a minute it is stopped with everything it started. `--bare` is not used, since it skips the keychain, where macOS keeps the login. The collector still calls no provider itself: Claude Code reads the usage, as `codex app-server` reads Codex's limits. A login without usage limits (an API key, a Console login, a cloud provider) is not asked, nor a home with nobody logged in, nor the Claude app's agent-mode homes. When the read fails, the cache is read as it is, and the source is partial with `claude /usage:` and why, such as `no answer in time`, or that this Claude Code needs an update. A read that failed with a cache fresh anyway, as when the person's own session wrote it meanwhile, is not a problem. A request Claude refused for a full window, found in its transcripts, makes that window read 100% until it resets, when the cache is older (see "Claude quota freshness").
