...
- **Claude quota freshness.** Claude Code writes its usage cache only when it runs, so an idle machine shows an old reading with its age. A request Claude refused for a full window is in the transcript with `quotaLimits`: that window was full then and stays full until `resetsAt`. The known window names are `five_hour`, `seven_day`, `seven_day_opus`, and `seven_day_sonnet`, and others are skipped. A refusal counts while its window has not reset and when it is newer than the account's reading. It becomes the reading of the account its session is attributed to: that window alone at 100%, as of the refusal, with source `rejection`. The older reading's other windows are not carried along, so none looks newer than it is, and that older reading comes back once the window resets. A refusal from before the previous run counts only when the ledger gives its whole session to the account, so a later login never takes an earlier account's refusal. A session with no tokens of its own is not read, so its refusal is not used. A fresh reading without any refusal would mean driving `claude` in tmux and scraping `/usage`. That is fragile and deferred.
