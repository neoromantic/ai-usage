...
- **Binary and scheduling**
  ...
  - `install.sh` puts `ai-usage` where it runs by name. An upgrade replaces the earlier binary where it is: `~/.local/bin/ai-usage`, or the one found on `PATH` when its folder is writable. So there is never a second copy, and the scheduler entry keeps pointing at it. A new install goes into the first of `~/.local/bin`, `~/bin`, `/opt/homebrew/bin`, and `/usr/local/bin` that is on `PATH` and writable, never into a folder another tool owns, such as `~/.cargo/bin`. When none fits, it installs into `~/.local/bin` and adds one marked block to the profile of the shell `$SHELL` names. For zsh that is `.zshrc`. For bash on macOS it is the first of `~/.bash_profile`, `~/.bash_login`, and `~/.profile` that exists, since a login shell reads only that one. For bash on Linux it is `~/.bashrc`, for fish `conf.d/ai-usage.fish`, and otherwise `~/.profile`. Running it again adds nothing. `AI_USAGE_BIN_DIR` still picks the folder and edits no profile, and `AI_USAGE_NO_MODIFY_PATH=1` prints the old hint instead. As root, a new install goes into `/usr/local/bin`, so the container docs pass `AI_USAGE_BIN_DIR` for a root install.
