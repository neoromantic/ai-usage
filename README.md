...
The menu bar shows a gauge and the percent left of the fullest window among the subscriptions logged in on this Mac: each one's main window, and any other that limits the account more, such as a 5-hour window that is out, since any full window stops work. Those other windows are the ones the terminal gives rows of their own. The popover has this machine's name and how many devices the team has, whether collection, the relay, and self-update are healthy, what needs attention, and four tabs: Subscriptions, each account's windows with when they reset and where they are headed, and a line for the accounts without a reading; Usage, the matrix of machines against subscriptions over a period you pick, in whole millions of tokens or in percent; Devices, each machine's status; and Projects, one row per repository with its subfolders and worktrees. With this machine alone in the team, there is no Devices, and Usage is a row per account, with today, 7, 30, and 90 days. Details are in tooltips. Refresh collects now.
